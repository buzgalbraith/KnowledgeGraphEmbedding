...
CUDA_VISIBLE_DEVICES=$GPU_DEVICE python -u $CODE_PATH/validate_run.py --do_train \
    --cuda \
    --do_valid \
    --do_test \
...
CUDA_VISIBLE_DEVICES=$GPU_DEVICE python -u $CODE_PATH/validate_run.py --do_valid --cuda -init $SAVE --triplet_type $TRIPLET_TYPE --all_datapath $OVERALL_DATA_PATH --negative_sample_type_test $negative_sample_type_test --test_datapath $TEST_DATA_PATH --hidden_dim $HIDDEN_DIM --AUC $AUC
...
CUDA_VISIBLE_DEVICES=$GPU_DEVICE CUDA_LAUNCH_BLOCKING=1 python -u $CODE_PATH/validate_run.py --do_test --cuda -init $SAVE --data_path $TEST_DATA_PATH --triplet_type $TRIPLET_TYPE --all_datapath $OVERALL_DATA_PATH --negative_sample_type_test $negative_sample_type_test --test_datapath $TEST_DATA_PATH --hidden_dim $HIDDEN_DIM \
