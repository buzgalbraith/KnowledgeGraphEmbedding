...
def parse_args(args=None):
    ...
    ## buz modification - we are changing datapath to test_datapath (that is the triplet type in this case) and adding all_datapath 
    parser.add_argument('--test_datapath', type=str, default=None, help='path to the directory with the test data (ie the triplet type we are looking at )')
    parser.add_argument('--all_datapath', type=str, default=None, help='path to the directory with all the training data defaults to ./data/MSK')
    ...
    parser.add_argument('--AUC', type=str, default='false', help='whether to calculate AUC', required=False)
...
def read_triple(file_path, entity2id, relation2id):
    ...
    with open(file_path) as fin:
        for line in fin:
            ...
            triples.append((entity2id[h.strip()], relation2id[r], entity2id[t.strip()]))
    return triples
...
def main(args):
    ...
    if args.init_checkpoint:
        ...
        if args.triplet_type != 'all' and args.negative_sample_type_test != 'uniform':
            ...
            train_triples = utils.reset_triplets(train_triples, new_entity2id, new_relation2id)
            valid_triples = utils.reset_triplets(valid_triples, new_entity2id, new_relation2id)
            test_triples = utils.reset_triplets(test_triples, new_entity2id, new_relation2id)
            all_true_triples = utils.reset_triplets(all_true_triples, new_entity2id, new_relation2id)
            ## want to reset everything in terms of the new entity2id and relation2id
