...
def relation_to_triplet_type(triplets_to_consider: list)->None:
    ...
    df = pd.DataFrame([overall_dict.values(), overall_dict.keys()]).T
    df.fillna(' NA', inplace=True)
    df.to_csv(GENERATED_DATA_PATH + "/relations_to_triplet_type.txt", sep='\t', index=False, header=False)
...
if __name__ == "__main__":
    ...
    relation_to_triplet_type(triplets_to_consider)
